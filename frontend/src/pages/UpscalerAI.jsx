...
export default function UpscalerAI() {
  ...
  const upscaleImage = async () => {
    ...
    try {
      const res = await axios.post("http://127.0.0.1:8000/upscale-image", formData);
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        <div className="mb-8 rounded-[32px] border border-slate-200/80 bg-white/90 p-8 shadow-soft dark:border-slate-800/80 dark:bg-slate-900/85">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-sm uppercase tracking-[0.35em] text-brand-600">Photo Upscaler</p>
              <h1 className="mt-2 text-3xl font-semibold tracking-tight sm:text-4xl">AI Photo Upscaler</h1>
              <p className="mt-4 max-w-2xl text-slate-500 dark:text-slate-400">
                Upscale low-resolution images quickly with a sharp, enhanced result.
              </p>
            </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr,1fr]">
          <div className="rounded-[32px] border border-slate-200/80 bg-white/90 p-8 shadow-soft dark:border-slate-800/80 dark:bg-slate-900/85">
            <label className="block text-sm font-semibold text-slate-900 dark:text-slate-100">Pick an image</label>
            ...
            <button onClick={upscaleImage} disabled={loading} className="mt-6 inline-flex w-full items-center justify-center rounded-3xl bg-brand-600 px-6 py-4 text-sm font-semibold text-white transition hover:bg-brand-700 disabled:cursor-not-allowed disabled:opacity-60">
              Upscale Image
            </button>
            {loading && <p className="mt-4 text-sm text-slate-500 dark:text-slate-400">Upscaling...</p>}
          </div>
          ...
          <div className="space-y-6">
            ...
            {result && (
              <div className="rounded-[32px] border border-slate-200/80 bg-slate-50 p-6 shadow-soft dark:border-slate-800/80 dark:bg-slate-950/70">
                <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Upscaled</h2>
                <img src={`data:image/png;base64,${result}`} alt="upscaled" className="mt-4 w-full rounded-3xl object-cover" />
                <a href={`data:image/png;base64,${result}`} download="upscaled-image.png" className="mt-6 inline-flex rounded-3xl bg-brand-600 px-6 py-3 text-sm font-semibold text-white transition hover:bg-brand-700">
                  Download Image
                </a>
              </div>
            )}
