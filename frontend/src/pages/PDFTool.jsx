...
export default function PDFTool() {
  ...
  const handleConvert = async () => {
    ...
    try {
      const response = await api.post("/pdf-to-docx", formData, {
        responseType: "blob",
      });
      ...
      if (err.code === "ECONNABORTED") {
        ...
        setError("Conversion failed. Please make sure your file is a valid PDF and try again.");
      }
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-slate-50 py-10 text-slate-950 dark:bg-slate-950 dark:text-slate-100">
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="rounded-[32px] border border-slate-200/80 bg-white/90 p-8 shadow-soft dark:border-slate-800/80 dark:bg-slate-900/85">
          ...
          <input type="file" accept=".pdf" onChange={(e) => { setFile(e.target.files[0]); setError(null); setSuccess(false); }} className="mt-4 w-full rounded-3xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-brand-500 focus:ring-4 focus:ring-brand-100 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 dark:focus:border-brand-400 dark:focus:ring-brand-500/10" />
          ...
          {loading && <p className="mt-4 text-sm text-slate-500 dark:text-slate-400">Converting your PDF — this may take a moment…</p>}
